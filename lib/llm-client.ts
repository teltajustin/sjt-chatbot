// lib/llm-client.ts
// v10: gemini-3.1-flash-lite-preview (정확한 모델명), GPT-4o, Claude
...
const CHAT_PARAMS = { temperature: 0.85, top_p: 0.92, frequency_penalty: 0.4, presence_penalty: 0.3 };
const GEMINI_PARAMS = { temperature: 0.85, topP: 0.92, topK: 40, frequencyPenalty: 0.4, presencePenalty: 0.3 };

// Gemini를 첫 번째로 반환 → 기본 선택됨
...
async function callAnthropicRaw(sys: string, user: string, max: number) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({ model: LLM_CONFIGS.anthropic.model, max_tokens: max, system: sys, messages: [{ role: "user", content: user }], temperature: CHAT_PARAMS.temperature, top_p: CHAT_PARAMS.top_p }),
  });
...
async function callOpenAIRaw(sys: string, user: string, max: number) {
  const res = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: LLM_CONFIGS.openai.model, max_tokens: max, messages: [{ role: "system", content: sys },{ role: "user", content: user }], temperature: CHAT_PARAMS.temperature, top_p: CHAT_PARAMS.top_p, frequency_penalty: CHAT_PARAMS.frequency_penalty, presence_penalty: CHAT_PARAMS.presence_penalty }),
  });
...
async function callGeminiRaw(sys: string, user: string, max: number) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: { maxOutputTokens: max, temperature: GEMINI_PARAMS.temperature, topP: GEMINI_PARAMS.topP, topK: GEMINI_PARAMS.topK, frequencyPenalty: GEMINI_PARAMS.frequencyPenalty, presencePenalty: GEMINI_PARAMS.presencePenalty },
    }),
  });
