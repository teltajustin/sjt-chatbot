  ...
  return(<div style={{height:"100vh",display:"flex",flexDirection:"column",background:"#fff"}}>
    ...
    <div style={{borderBottom:"1px solid #e8e8e8",padding:"0 20px",height:50,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        ...
        <span style={{fontSize:"0.9375rem",fontWeight:700,color:"#1d1c1d"}}><span style={{color:"#616061",fontWeight:400}}>#</span> {scenario?.title}</span>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <div style={{display:"flex"}}>{personas.map((p,i)=>(<div key={p.id} title={p.name} style={{width:24,height:24,borderRadius:5,background:getAvatarBg(p.name),display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:10,fontWeight:700,marginLeft:i>0?-6:0,border:"2px solid #fff",position:"relative",zIndex:3-i}}>{p.name.charAt(0)}</div>))}</div>
